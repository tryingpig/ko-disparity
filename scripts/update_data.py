...
# -*- coding: utf-8 -*-
"""
한국 섹터 ETF 100일 이격도 트래커 — 데이터 수집/계산 스크립트

한국 주요 지수(코스피·코스닥)와 섹터 ETF의 일봉을 yfinance로 받아
100일 이동평균(SMA)과 이격도를 구하고, 고정 임계값으로 구간을 판정해
data/summary.json (메인 순위표용)과 data/{종목코드}.json (종목별 시계열)을 생성한다.
...
# 추적 대상(섹터 ETF + 지수)은 data/sectors.json에서 읽는다.
# 이 파일은 Notion 'ETF Sector DB'에서 sync_from_notion.py로 생성된다(코드 하드코딩 → 외부 config).
# 각 항목 구조는 예전 하드코딩과 동일: ticker, slug, name_ko, name_en, theme.
#   sectors : yfinance 심볼 '<종목코드>.KS', slug=종목코드, name_ko=섹터명(주 라벨), theme=대표ETF명(부제)
#   indices : yfinance 심볼 '^...', slug='^' 뺀 값
def load_universe():
    """data/sectors.json에서 (SECTORS, INDICES) 목록을 읽는다."""
    cfg_path = DATA_DIR / "sectors.json"
    if not cfg_path.exists():
        sys.exit(
            f"{cfg_path} 없음 — Notion 동기화(scripts/sync_from_notion.py)를 먼저 실행하세요."
        )
    cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
    return cfg.get("sectors", []), cfg.get("indices", [])


SECTORS, INDICES = load_universe()
